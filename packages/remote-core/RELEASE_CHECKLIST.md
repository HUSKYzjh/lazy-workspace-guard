...
- [ ] Run `npm run package:bridge` and `npm run package:suite`; install the bridge locally and the core in a Remote-SSH window before testing their command handshake.
...
- [ ] 运行 `npm run package:bridge` 和 `npm run package:suite`；在本机安装桥接、在 Remote-SSH 窗口安装核心后，验证二者的命令握手。
