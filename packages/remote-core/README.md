...
Install **Lazy Workspace Guard** from Marketplace. It is the Suite entry point and installs two coordinated extensions in their correct locations:

- **Lazy Workspace Guard Remote Core** runs on the connected SSH host and performs bounded directory reads.
- **Lazy Workspace Guard Local Bridge** runs locally and resolves the selected profile from your local OpenSSH config.
...
For manual VSIX installation, install `lazy-workspace-guard-remote-core-<version>.vsix` in the Remote-SSH target and `lazy-workspace-guard-local-bridge-<version>.vsix` under **Local – Installed**, then reload the Remote-SSH window.
...
推荐直接从 Marketplace 安装 **Lazy Workspace Guard**。它是 Suite 入口，会在正确的位置安装两个协作扩展：

- **惰性工作区守卫：远端核心**运行在已连接的 SSH 主机上，负责有界目录读取。
- **惰性工作区守卫：本机桥接**运行在本机，负责解析本机 OpenSSH config 中选定的配置。
...
手动安装 VSIX 时，请在 Remote-SSH 目标安装 `lazy-workspace-guard-remote-core-<version>.vsix`，并在 **Local – 已安装** 下安装 `lazy-workspace-guard-local-bridge-<version>.vsix`，然后重载 Remote-SSH 窗口。
